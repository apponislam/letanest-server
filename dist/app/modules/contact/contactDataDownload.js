...
exports.contactDownloader = void 0;
...
const downloadContactsExcel = (0, catchAsync_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { month, year, status } = req.query;
    if (!year) {
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, "Year is required for downloading contact data");
    }
    // Parse year
    const yearStr = Array.isArray(year) ? (year.length > 0 ? String(year[0]) : null) : String(year);
    if (!yearStr) {
    ...
    // Fetch contacts based on filter
    const contacts = yield contact_model_1.Contact.find(filter).sort({ createdAt: -1 });
    console.log(`Found ${contacts.length} contacts for download`);
    if (contacts.length === 0) {
        throw new ApiError_1.default(http_status_1.default.NOT_FOUND, "No contacts found for the selected period and filters");
    }
    // Create Excel workbook
